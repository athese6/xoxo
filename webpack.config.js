...
const clientType = config.getClientType();
...
const entries = Object
    ...
    .reduce((obj, key) => {
        ...
        if (entry) {
            obj = obj.concat([].concat(entry).map(item => path.normalize(`${inputDir}/${item}`)));
        }
        return obj;
    }, []);

const entry = initialsEntries.concat(entries);
const webpackConfig = {
    mode: env || "development",
    watch: config_webpack.hotReload,
    entry: config_webpack.entries ? initialsEntries.concat(entries) : [],
    output: {
        path: outputDir,
    ...
    plugins: ([
            ...
            new webpack.DefinePlugin({
                ...
                "process.mode": env || "development",
                "process.clientType": clientType || "web",
            ...
            // config_webpack.splitVendor ? new webpack.optimize.SplitChunksPlugin(config_webpack.optimization) : []
        ).concat(
    ...
    optimization: config_webpack.optimization,
